...
def main():
    if not (
        ...
        if os.path.exists(postgresql.postgresql_conf_path()):
            hookenv.status_set(
                "blocked",
                "PostgreSQL config from previous "
                "install found at {}"
            ...
            hookenv.status_set(
                "blocked",
                "PostgreSQL database from previous "
                "install found at {}"
...
def emit_deprecated_option_warnings():
    ...
    if deprecated:
        hookenv.log(
            "Deprecated configuration settings in use: {}"
...
@when("postgresql.cluster.created")
@when_any("postgresql.replication.has_master", "postgresql.replication.failover")
...
def install_administrative_scripts():
    ...
    helpers.makedirs(backups_dir, mode=0o750, user="postgres", group="postgres")
    ...
    if not os.path.exists(logs_dir):
        helpers.makedirs(logs_dir, mode=0o755, user="postgres", group="postgres")

...
def update_default_wal_e_env_dir():
    update_wal_e_env_dir(wal_e_env_dir(), hookenv.config().get("wal_e_storage_uri", ""))
...
def wal_e_prune_command():
    config = hookenv.config()
    return (
        "/snap/bin/wal-e.envdir {} /snap/bin/wal-e "
...
def wal_e_list_backups(envdir=None):
    ...
    details = [{r[0][i]: r[j][i] for i in range(len(r[0]))} for j in range(1, len(r))]
    return details
